...
const TX_TYPE_MAP: Record<Transaction['type'], { label: string; variant: 'success' | 'warning' | 'danger' | 'default' }> = {
  deposit: { label: 'Yükleme', variant: 'success' },
  withdrawal: { label: 'Çekim', variant: 'warning' },
  purchase: { label: 'Alışveriş', variant: 'default' },
  refund: { label: 'İade', variant: 'magenta' as 'default' },
};
...
export default function DashboardWalletPage() {
  ...
  const [transactions] = React.useState<Transaction[]>(MOCK_TX);

  const balances = [
    { code: 'TRY', name: 'Türk Lirası', symbol: '₺', value: user?.wallet.balanceTry ?? 0, color: 'cyan' },
    { code: 'USD', name: 'Amerikan Doları', symbol: '$', value: user?.wallet.balanceUsd ?? 0, color: 'lime' },
    { code: 'EUR', name: 'Euro', symbol: '€', value: user?.wallet.balanceEur ?? 0, color: 'magenta' },
    { code: 'USDT', name: 'Tether', symbol: '₮', value: user?.wallet.balanceUsdt ?? 0, color: 'cyan' },
  ];
          ...
          <Card key={b.code} className="group">
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-medium uppercase tracking-wider text-white/60">{b.code}</p>
                  <p className="mt-1 text-xs text-white/50">{b.name}</p>
                  <p className="mt-3 font-orbitron text-2xl font-bold text-white">
                    {b.symbol}
                    {b.value.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </p>
                </div>
        ...
        <CardContent className="p-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="filter" className="text-xs text-white/60">Filtre:</Label>
              <Select
                id="filter"
                value={filterType}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredTx.map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} className="border-b border-cyber-cyan/10 hover:bg-cyber-cyan/5">
                        <td className="py-3 pr-4 text-white/70">{tx.date}</td>
                        <td className="py-3 pr-4">
                          <Badge variant={m.variant} size="sm">{m.label}</Badge>
                        </td>
                        <td className={`py-3 pr-4 text-right font-medium ${isPositive ? 'text-cyber-lime' : 'text-white'}`}>
                          {isPositive ? '+' : '-'}₺{tx.amount.toLocaleString()}
                        </td>
                        <td className="py-3 pr-4 text-right text-white/70">₺{tx.balanceAfter.toLocaleString()}</td>
