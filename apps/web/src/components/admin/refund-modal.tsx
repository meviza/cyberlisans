  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch(`/admin/orders/${orderId}/refund`, {
        method: 'POST',
        body: JSON.stringify({ amount: amtNum, reason: reason.trim() }),
      });
      reset();
      onSuccess?.();
      onClose();
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content
          className={cn(
            ...
            'rounded-lg border border-cyber-purple/30 bg-cyber-darker p-6 shadow-2xl focus:outline-none',
          )}
        >
          <div className="mb-4 flex items-center gap-3">
            <div className="rounded-md border border-cyber-purple/30 bg-cyber-purple/10 p-2 text-cyber-purple">
